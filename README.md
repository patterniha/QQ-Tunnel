...
It just acts as a UDP forwarder, so for accessing internet, you need a reliable data transfer over UDP like: wireguard/hysteria/kcp/...
...
`max_domain_len`: maximum length of final domain length that resolver allow to pass, (count without trailing dot, for example the length of a.b.com is 7), in theory, resolvers should support up to 253 domain length, but some resolvers limited to lower value (99/101/151/...).
...
also, dns_Out2Iran.txt file contains some scanned dns to use for outside-to-Iran and there are many more options for Iran-to-outside dns.
using multiple dns improve speed, but be careful that all DNS must be active, otherwise the tunnel will not function properly.
